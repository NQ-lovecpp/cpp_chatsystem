...
/**
 * Check if the client is configured for OpenRouter (Responses API has limited support).
 */
function isOpenRouter(client: OpenAI): boolean {
  const baseURL =
    typeof client.baseURL === 'string'
      ? client.baseURL
      : (client as any).baseURL?.toString?.() ?? '';
  return baseURL.includes('openrouter.ai');
}

/**
 * Normalize input for OpenRouter: filter unsupported item types (e.g. reasoning),
 * merge instructions as first system message, ensure compatible format.
 */
function normalizeInputForOpenRouter(
  input: OpenAI.Responses.ResponseInputItem[],
  systemInstructions?: string,
): OpenAI.Responses.ResponseInputItem[] | string {
  ...
  if (filtered.length === 0) {
    return systemInstructions?.trim() || 'Continue.';
  }

  const withSystem: OpenAI.Responses.ResponseInputItem[] = [];
  ...
  withSystem.push(...filtered);

  if (withSystem.length === 1) {
    const first = withSystem[0] as { role?: string; content?: string };
...
/**
 * Build minimal request body for OpenRouter (strips unsupported params).
 */
function buildOpenRouterRequestData(
  base: Record<string, any>,
  model: string,
  input: OpenAI.Responses.ResponseInputItem[] | string,
  stream: boolean,
  tools: OpenAI.Responses.Tool[],
  toolChoice: ToolChoice | undefined,
  shouldSendTools: boolean,
  shouldOmitToolChoice: boolean,
): Record<string, any> {
  return {
    model,
    input,
    ...
    stream,
    ...(shouldSendTools && tools.length > 0 ? { tools } : {}),
    ...(!shouldOmitToolChoice && toolChoice
...
export class OpenAIResponsesModel implements Model {
  ...
  > {
    const rawInput = getInputItems(request.input);
    ...
    if (isOpenRouter(this.#client)) {
      const instructions = normalizeInstructions(request.systemInstructions);
      const input = normalizeInputForOpenRouter(rawInput, instructions);
      requestData = buildOpenRouterRequestData(
        {
